...
const UseCart = () => {
    ...
    useEffect(() => {
        fetch(`https://hidden-castle-93174.herokuapp.com/cart/${uid}`)
            .then(res => res.json())
            .then(data => {
                if (data.length) {
    ...
    function addToCart(dt) {
        ...
        if (isHave) {
            ...
            fetch('https://hidden-castle-93174.herokuapp.com/booking/add', {
                method: "POST",
    ...
    function remove(id) {
        ...
        const url = `https://hidden-castle-93174.herokuapp.com/booking/add/${id}`;
        fetch(url, {
            method: 'DELETE'
        })
            .then(res => res.json())
